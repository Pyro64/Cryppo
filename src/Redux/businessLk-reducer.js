    ...
    switch (action.type) {
        case FILTER_VALUTE: 
            ...
            const operation = initialState.operationData.operation.filter((item)=>{
                if (QUERY && DATERANGE){
                    return item.check.includes(action.value) && item.currencyPay === CURRENCY.value &&
                                new Date(item.data).valueOf() <= new Date(DATERANGE[1]).valueOf() && new Date(item.data).valueOf() >= new Date(DATERANGE[0]).valueOf()
                }
                else{
                    if(QUERY){
                        return item.check.includes(action.value) && item.currencyPay === CURRENCY.value
        ...
        case FILTER_DATE:
            ...
            return{
                ...
                operationData: {
                    ...
                    operation:initialState.operationData.operation.filter((item)=>{
                        if (CURRENCY && QUERY){
                            return new Date(item.data).valueOf() <= new Date(action.value[1]).valueOf() && new Date(item.data).valueOf() >= new Date(action.value[0]).valueOf()
                             && item.currencyPay === CURRENCY.value && item.check.includes(QUERY)
                        }
                        else{
                            if(CURRENCY){
                                return new Date(item.data).valueOf() <= new Date(action.value[1]).valueOf() && new Date(item.data).valueOf() >= new Date(action.value[0]).valueOf()
